...
class File(ABC):
    ...
    def copy_to(self, destination: str) -> "File":
        """
        Copies the file to a different path and returns a new instance of the same class with the updated path.

        :param destination: str
            The path where the file will be copied.
        :return: File
            A new instance of the same class with the updated path.
        :raises FileNotFoundError:
            If the source file does not exist.
        """
        if not os.path.isfile(self.filepath):
            raise FileNotFoundError(f"Source file does not exist: {self.filepath}")

        os.makedirs(os.path.dirname(destination), exist_ok=True)
        shutil.copy(self.filepath, destination)
        logger.info(f"Copied file from {self.filepath} to {destination}")

        # Return a new instance of the same class with the updated path
        return type(self)(filepath=destination, output_path=self.output_path)
