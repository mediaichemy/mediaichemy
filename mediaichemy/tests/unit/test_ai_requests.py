...
@pytest.mark.asyncio
async def test_ai_request_text(mock_providers):
    """
...
@pytest.mark.asyncio
async def test_ai_request_image(mock_providers):
    """
    Test ai_request for image generation using RunwareProvider.
    """
    result = await ai_request(media="image", prompt="Generate an image",
                              output_path="mediaichemy/tests/resources/temp_content/image.jpg")
    mock_providers["mock_runware"].assert_called_once_with("Generate an image",
                                                           output_path="mediaichemy/tests/resources/temp_content/image.jpg")
    assert isinstance(result, JPEGFile)
    assert result.filepath == "mediaichemy/tests/resources/temp_content/image.jpg"


@pytest.mark.asyncio
async def test_ai_request_video(mock_providers):
    """
    Test ai_request for video generation using MinimaxProvider.
    """
    result = await ai_request(media="video", prompt="Generate a video",
                              output_path="mediaichemy/tests/resources/temp_content/video.mp4",
                              input_path="mediaichemy/tests/resources/temp_content/image.jpg")
    mock_providers["mock_minimax"].assert_called_once_with("Generate a video",
                                                           output_path="mediaichemy/tests/resources/temp_content/video.mp4",
                                                           input_path="mediaichemy/tests/resources/temp_content/image.jpg")
    assert isinstance(result, MP4File)
    assert result.filepath == "mediaichemy/tests/resources/temp_content/video.mp4"


@pytest.mark.asyncio
async def test_ai_request_speech(mock_providers):
    """
    ...
    result = await ai_request(media="speech", prompt="Generate speech",
                              output_path="mediaichemy/tests/resources/temp_content/speech.mp3")
    mock_providers["mock_elevenlabs"].assert_called_once_with(
        "Generate speech",
        output_path="mediaichemy/tests/resources/temp_content/speech.mp3")
    assert isinstance(result, MP3File)
    assert result.filepath == "mediaichemy/tests/resources/temp_content/speech.mp3"
